...
<div
	class={cn(
		'fixed left-0 top-16 flex flex-col gap-8 border-r bg-light-300 border-light-900',
		'p-10 pt-16 <md:hidden pr-0',
		'lg:(pl-29 w-90)'
	)}
			...
			<ul class="flex flex-col gap-0.5 font-light">
				{#each sections[section] as item}
					...
					<li class="relative">
						...
						{#if href === data.slug}
							<span
								class="absolute -right-1.5 lg:-right-4.5 top-1/2 transform -translate-y-1/2 rotate-45 pointer-events-none w-3 h-3 bg-white border-l border-light-900 border-b"
							></span>
						{/if}
					</li>
				{/each}
